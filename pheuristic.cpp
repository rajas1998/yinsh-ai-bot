// our attempt for the first general file

//select and movw vagerah and reverse from ta_v9_reversi == done
// minimax from same and then generalize it == done

//add sort manually, consult side by side ta v9 sort prune
#include <iostream>
#include <stdio.h>
#include<vector>
#include <stdlib.h>
...
vector<pair<int,int> > other_rings; //position of other players rings

// int kay;
int rings_left_to_place ; //six - gie it value after scanning
...
vector<vector<int> > find_k_in_a_specific_direction(int k, int dir_num ,pair<int,int> arr[], Config& config){
	...
	//function that tells us with a few checks if it is pakka not possible but cant say if its true
	last_type=-1;
	type=-1;
...
{  
  ...
  //mostly isse pehle hi return ho chuka hoga//so lets print something
  ...
  for (int ind = 0; ind < mapped_evaluation.size(); ++ind)
  {
    ...
    if (mapped_evaluation[ind].second.size() == 10){
      ...
      beta = min_float(beta, temp_utility);
      if (alpha >= beta) {
          if(depth>1 ) prunes++; //phark pada kya, cause second last layer mei pruning ho hi nhi rahi
          return temp_utility;
      }
    }
    else if (mapped_evaluation[ind].second.size() == 4){
...
void play(){
	...
	//initialise threatened vala vector
	int i,j;
	vector<int> f_row;
	f_row.push_back(0);
	...
	//Game loop has to start from here
	int num_moves = 0;
	int counter = 0;
	while (true)
	{
